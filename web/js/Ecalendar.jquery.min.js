$.fn.ECalendar = function (config) {
    var con = {offset: [0, 0], type: 1, skin: false, stamp: true, step: 10};
    config = $.extend(con, config);
    var _this = $(this), ECalendar, day, imonth, iday, iyear, ilast, itimebox, select, getFullYear, getMonth, getDate,
        weekday, getHour, getMinutes, oper, currenttime, x, y, mx, my, timestep = config.step, drag, input, dragnum,
        isinit = false, pointer, callback, formatTime = config.format ? config.format : "yyyy-mm-dd hh:ii",
        ...
        if (drag) {
            ...
            if (drag[0].tagName == "LI") {
                if (Math.abs(my) > 1 || Math.abs(mx) > 1) {
                    ...
                    if (ax < 0 && ay > 0) a = 180 + a;
                    if (ax < 0 && ay <= 0) a = 180 + a;
                    if (ax >= 0 && ay < 0) a = 360 + a;
                    var atime = a / 30;
    ...
    function setData(date) {
        ...
        var val = config.format.replace('yyyy', getFullYear).replace('mm', getMonth).replace('dd', getDate).replace('hh', getHour < 10 ? "0" + getHour : getHour).replace('ii', getMinutes < 10 ? "0" + getMinutes : getMinutes);
        _this.attr("data-ec", date);
        _this.val(val);
        stamp = val.replace(/-/g, "/")
        stamp = Date.parse(stamp) / 1000
        ECalendar.data("thistime", stamp);
        if (config.stamp) input.val(stamp); else input.val(val);
    }
